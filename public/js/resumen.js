document.getElementById('btnEnviar').addEventListener('click', async () => {
    ...
    if (!textoUsuario.trim()) {
        contenedor.textContent = "Por favor, introduce un texto válido para resumir.";
        return;
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (resultadoFinal.status === 'failed') throw new Error('El servidor no pudo procesar el resumen.');
        ...
        if (!frasesResumen || frasesResumen.length === 0) {
            contenedor.innerHTML = "<p>No se pudo generar un resumen para el texto provisto.</p>";
            return;
        }
